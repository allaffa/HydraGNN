...
"""
data.x[:,0] and data.x[:,1] contain the two quantities that are supposed to be known for a given node
data.x[:,2] is binary columns. 1 means that P is known, 0 means that P is not known
data.x[:,3] is binary columns. 1 means that Q is known, 0 means that Q is not known
data.x[:,4] is binary columns. 1 means that V is known, 0 means that V is not known
data.x[:,5] is binary columns. 1 means that theta is known, 0 means that theta is not known
    ...
    def __init__(self, dirpath, var_config, dist=False):
        ...
        for directory in directories_list:
            ...
            for csv_file in local_csv_files_list:
                ...
                node_features = torch.empty((temp_node_features.shape[0], 2), dtype=torch.float32)

                S_base_tensor = torch.ones((temp_node_features.shape[0], 1), dtype=torch.float32) * S_base
                ...
                node_mask = torch.zeros((temp_node_features.shape[0], 3), dtype=torch.int)
                ...
                for idx, bus_type in enumerate(data_csv["Bus Type"]):
                    if bus_type == "Slack":
                        ...
                        node_features[idx, 0] = temp_node_features[idx, 2]
                        node_features[idx, 1] = temp_node_features[idx, 3]
                        node_mask[idx] = torch.tensor([1, 0, 0], dtype=torch.int)
                    elif bus_type == "PQ Bus":
                        #node_mask[idx] = torch.tensor([1, 1, 0, 0], dtype=torch.int)
                        node_features[idx, 0] = temp_node_features[idx, 0]
                        node_features[idx, 1] = temp_node_features[idx, 1]
                        node_mask[idx] = torch.tensor([0, 1, 0], dtype=torch.int)
                    elif bus_type == "PV Bus":
                        #node_mask[idx] = torch.tensor([1, 0, 1, 0], dtype=torch.int)
                        node_features[idx, 0] = temp_node_features[idx, 0]
                        node_features[idx, 1] = temp_node_features[idx, 2]
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--inputfile", help="input file", type=str, default="power_grid_no_masking.json"
    )
