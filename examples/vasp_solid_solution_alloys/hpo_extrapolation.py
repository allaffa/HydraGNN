...
def read_outcar(file_path):
    ...
    assert len(result_supercell) == len(
        result_atomic_structure_sections), "The length of the list of supercells " + str(
        len(result_supercell)) + " differs from the length of the list of atomistic structures " + str(
        len(result_atomic_structure_sections))
        ...
        for name in iterate_tqdm(os.listdir(os.path.join(dirpath, "../", "pure_elements")), verbosity_level=2,
            ...
            for file_name in outcar_files:
                ...
                file_path = os.path.join(
                    os.path.join(dirpath, "../", "pure_elements", name, name + '128', 'case-1', file_name))
                        ...
                        if filename == "OUTCAR" or filename == "OUTCAR-bis":
                            ...
                            try:
                                ...
                                temp_dataset, _ = read_outcar(
                                    os.path.join(dir_name, subdir_name, subsubdir_name) + '/' + filename)
                                ...
                                print(self.rank, "Exception:", os.path.join(subdir_name, subsubdir_name, filename), e,
                                      file=sys.stderr)
                                # traceback.print_exc()
                                pass
