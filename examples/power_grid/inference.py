...
if __name__ == "__main__":
    ...
    checkpoint_path = os.path.join(dirpwd, "logs", modelname, f"{modelname}.pk")
    ...
    for output_name, output_type, output_dim in zip(
        ...
        for data in tqdm(test_data_loader):
            ...
            # Calculate MAE per node (mean, not sum)
            num_nodes = true.shape[0]
            test_MAE += (torch.norm(predicted - true, p=1).item() / num_nodes) / len(testset)
